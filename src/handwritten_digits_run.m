...
for ds_idx = 1:size(data_splits,1)
    ...
    figure
    [tra, val, tes] = get_data(tes_p,val_p,'handwritten');
    ...
    for i = 1:length(outlier_costs)
        for j = 1:length(krnl_params)
            cv_err = zeros(3,1);
            for cvi = 1:3
               [theta,cv_err(cvi),K,K0] = train_mclass_kernel_SVM(tra(cvi), val(cvi), @rbf_k, outlier_costs(i), krnl_params(j));
            end
            cv_err = mean(cv_err);
            err(i,j) = cv_err;
            if(cv_err<min_err)
    ...
    savefig(['handwritten' int2str(tes_p)  '_'  int2str(val_p) '.fig']);
    
    [theta,cv_err(1),K,K0] = train_mclass_kernel_SVM(tra(1), val(1), @rbf_k, C, sigma);
    ...
    disp(['training%- ' int2str(100 - tes_p) ', validation%- ' num2str(val_p) ', C- ' num2str(C) ', sigma- ' num2str(sigma) ', Cross Validation accuracy- ' num2str(1-min_err)  ', Test accuracy- ' num2str(1-tes_err)])
    res_i = res_i +1;
end
